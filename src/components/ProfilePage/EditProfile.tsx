...
const EditProfile = () => {
    ...
    const { data: gameEntriesData, isLoading: isFetching, refetch: refetchGameEntries } = useGetUserGameEntriesQuery();
    const [createGameEntries, { isLoading: isCreating }] = useCreateGameEntriesMutation();
    const [updateGameEntry, { isLoading: isUpdating }] = useUpdateGameEntryMutation();
    const [deleteGameEntry, { isLoading: isDeleting }] = useDeleteGameEntryMutation();
    const [updateUser, { isLoading: isUpdatingUser }] = useUpdateUserMutation();

    // Local loading state for form submission
    const [isSubmitting, setIsSubmitting] = useState(false);

    const { register, control, handleSubmit, reset, formState: { isDirty } } = useForm<FormData>({
        defaultValues: {
            name: "",
    ...
    useEffect(() => {
        if (gameEntriesData?.data) {
            ...
            reset({
                ...
                gameEntries: gameEntriesData.data.length > 0
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            ...
            if (newEntries.length > 0) {
                await createGameEntries(newEntries.map(({ gameName, gameId }) => ({
                    gameName,
                    gameId
                }))).unwrap();
            }
    ...
    const isLoading = isFetching || isCreating || isUpdating || isDeleting || isUpdatingUser || isSubmitting;
                        ...
                        {fields.map((field, index) => (
                            <div key={field.id} className="flex items-center space-x-2 mb-2">
                                ...
                                <button
                                    ...
                                    disabled={isLoading}
                                >
                                    {isDeleting && field.id ? <Loader2 className="animate-spin h-4 w-4" /> : <X />}
                                </button>
                            </div>
                        ))}
