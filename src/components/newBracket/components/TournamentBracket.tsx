...
const TournamentBracket: React.FC<TournamentBracketProps> = ({ data }) => {
  ...
  const availableRounds = roundOrder.filter(round => 
    data[round] && data[round].length > 0
  );
  ...
  const findTournamentWinner = () => {
    if (data['Final'] && data['Final'].length > 0) {
      ...
      if (finalMatch.winnerId) {
        const winner = finalMatch.team1Id === finalMatch.winnerId 
          ? finalMatch.team1 
          : finalMatch.team2;
        return winner;
      }
    ...
    <div className="flex flex-col gap-4">
      ...
      {tournamentWinner && (
        <div className="bg-gradient-to-r from-amber-700 to-amber-600 p-4 rounded-lg shadow-lg border border-amber-500 mb-4">
          ...
          <p className="text-xl font-bold mt-2 text-center text-white">{tournamentWinner.name}</p>
          <p className="text-sm text-center text-amber-200">{tournamentWinner.fullName}</p>
        </div>
      )}
      ...
      {selectedMatch && (
        <MatchDetails match={selectedMatch} onClose={handleCloseDetails} />
      )}
    </div>
  );
};
